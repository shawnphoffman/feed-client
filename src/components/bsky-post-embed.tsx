...
import { faEarthAmericas, faInfoCircle, faPlayCircle, faShapes } from '@awesome.me/kit-ac8ad9255a/icons/classic/solid'
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'

import { Separator } from './ui/separator'
import Link from './bsky-post-link'
import { getRkey } from './bsky-utils'
...
export default function PostEmbed({ content, hideRecord }: Props) {
	...
	try {
		...
		if (AppBskyEmbedRecord.isView(content)) {
			...
			if (AppBskyEmbedRecord.isViewRecord(record)) {
				...
				if (pwiOptOut) {
					return <Info>The author of the quoted post has requested their posts not be displayed on external sites.</Info>
				}
		...
		if (AppBskyEmbedRecordWithMedia.isView(content) && AppBskyEmbedRecord.isViewRecord(content.record.record)) {
			return (
				<div className="flex flex-col gap-2">
					<PostEmbed content={content.media} hideRecord={hideRecord} />
					<PostEmbed
						content={{
		...
		<div className="w-full image overflow-hidden border flex flex-col items-stretch">
			...
			<div className="py-2 px-3 flex flex-col gap-1">
				...
				<p className="text-xs text-muted-foreground line-clamp-1 flex flex-row gap-1 items-center">
					<FontAwesomeIcon icon={faEarthAmericas} size="xs" />
	...
	return (
		<div className="w-full overflow-hidden image rotate-1 aspect-square relative" style={{ aspectRatio: `${aspectRatio} / 1` }}>
			...
			<div className="size-24 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-black/50 flex items-center justify-center">
				...
				<FontAwesomeIcon icon={faPlayCircle} className="object-cover size-3/5" />
			</div>
		</div>
	)
}

function StarterPackEmbed({ content }: { content: AppBskyGraphDefs.StarterPackViewBasic }) {
